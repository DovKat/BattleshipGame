...
const GameBoard: React.FC<GameBoardProps> = ({
  ...
  const [command, setCommand] = useState(""); // Track the command entered
  const [commandFeedback, setCommandFeedback] = useState(""); // Feedback message
  ...
  const handleShoot = async (
    ...
    try {
      const turnResult = await connection?.invoke(
        "MakeMove",
        "game-1",
        currentPlayerId,
        targetPlayerId,
      ...
      if (turnResult) {
        ...
        setGame((prevGame) => {
          ...
          if (targetPlayer) {
            AffectedCoordinates.forEach(
              (coord: { Row: number; Col: number }, index: number) => {
                const result = Results[index];
                targetPlayer.board.grid[coord.Row][coord.Col].isHit =
                  result === "Hit";
                targetPlayer.board.grid[coord.Row][coord.Col].isMiss =
                  result === "Miss";
              }
            );
          }
  ...
  const handleCommandSubmit = async () => {
    if (!connection) {
      setCommandFeedback("Connection to server is not available.");
      return;
    }
    ...
    if (commandParts.length < 3) {
      setCommandFeedback(
        "Invalid command. Format: <attackType> r<row> c<column>"
      );
      return;
    }

    const [attackType, rowPart, colPart] = commandParts;

    // Validate attack type (optional: match it with available attack types)
    const validAttackTypes = ["regular", "smallbomb", "bigbomb", "megabomb"];
    if (!validAttackTypes.includes(attackType.toLowerCase())) {
      setCommandFeedback(
        `Invalid attack type. Use one of: ${validAttackTypes.join(", ")}`
      );
      return;
    }

    // Validate row and column
    if (!/^r\d+$/.test(rowPart) || !/^c\d+$/.test(colPart)) {
      setCommandFeedback(
        "Invalid row or column format. Use r<row> c<column> (e.g., r5 c6)."
      );
      return;
    }

    const row = parseInt(rowPart.substring(1), 10);
    const col = parseInt(colPart.substring(1), 10);
    ...
    if (
      ...
      setCommandFeedback("Row and column must be between 0 and 9.");
      return;
    }
    // Get random enemy
    const enemy = getRandomEnemy(currentPlayerId || "");
    if (!enemy) {
      setCommandFeedback("No enemies available to attack.");
      return;
    }

    // Add gameId, currentPlayerId, and targetId to the command
    const enrichedCommand = `${command} gameId=${game?.gameId} currentPlayerId=${currentPlayerId} targetId=${enemy.id}`;

    // Send command to backend
    try {
      await connection.invoke("InterpretCommand", enrichedCommand);
      setCommandFeedback("Command sent successfully.");
    } catch (err) {
      console.error("Error sending command:", err);
      setCommandFeedback("Failed to send command.");
    }
  };
  ...
  const getTeammate = (currentPlayerId: string) => {
    if (currentPlayer && game && game.players) {
      return Object.values(game.players).find(
        (player) =>
          player.team === currentPlayer.team && player.id !== currentPlayerId
      );
    }
    return null;
  };

  const getRandomEnemy = (currentPlayerId: string): Player | null => {
    if (currentPlayer && game && game.players) {
                ...
                <>
                  ...
                  {game?.mode !== "Tournament" && (
                    <div className="attack-buttons">
                      <button onClick={() => setSelectedAttack("regular")}>
                        Regular Attack
                      </button>
                      <button onClick={() => setSelectedAttack("smallbomb")}>
                        Small Bomb
                      </button>
                      <button onClick={() => setSelectedAttack("bigbomb")}>
                        Big Bomb
                      </button>
                      <button onClick={() => setSelectedAttack("megabomb")}>
                        Super Bomb
                      </button>
                      ...
                      <div className="command-input">
                        <input
                          type="text"
                          placeholder="Enter your command (e.g.,regular r5 c6 | smallbomb r1 c7)"
                          defaultValue={""}
                          value={command}
                  ...
                  {currentPlayer && (
                    <>
                      <div
                        ...
                        style={{
                          backgroundColor: getTeamColor(currentPlayer.team),
                        }}
                      >
                        <div className="board-title">
                          {currentPlayer.name}'s Board
                        </div>
                        {currentPlayer.board && (
                          <Board
                      ...
                      {teammate && (
                        <div
                          ...
                          className="board-wrapper"
                          style={{
                            backgroundColor: getTeamColor(teammate.team),
                          }}
                        >
                          <div className="board-title">
                            {teammate.name}'s Board
                          </div>
                          {teammate.board && (
                            <Board
